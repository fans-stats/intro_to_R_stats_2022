...
```{r}
dataset$sex[dataset$sex == -1] <- NA
dataset$sex[dataset$sex == 2] <- NA
dataset$age[dataset$age == 105] <- NA
```

```{r}
dataset <- dataset %>% 
  mutate(sex = dplyr::if_else(sex == -1 | sex == 2, NA_real_, sex), 
         age = dplyr::if_else(age == 105, NA_real_, age))
```
...
dataset %>% 
  ...
  dplyr::summarise(mean_depress = my_mean(depress)) %>% 
  dplyr::ungroup()
```
...
dataset %>% 
  ...
  dplyr::summarise(mean_depress = my_mean(depress)) %>% 
  dplyr::ungroup() %>% 
  dplyr::mutate(mean_depress_sqrt = sqrt(mean_depress))
```


```{r}
dataset %>% 
  dplyr::group_by(treat, sex) %>% 
  dplyr::summarise(mean_depress = my_mean(depress)) %>% 
  dplyr::ungroup()
```
...
a) Use the appropriate R command to build a linear regression model that can be used to estimate and test the path a (effect of treatment on mediator). Report on what are the estimates and statistical significance of path a.

To estimate and test path a, we fit a simple linear regression of mediator (job_seek) on treatment (treat).

```{r}
dataset %>% 
  lm(formula = job_seek ~ treat) %>% 
  summary()
```
...
```{r}
dataset %>% 
  lm(formula = depress ~ treat + job_seek) %>% 
  summary()
```
...
bda::mediation.test(dataset$job_seek, 
                    dataset$treat, 
                    dataset$depress)
```
...
```{r}
psych::mediate(data = dataset, 
               depress ~ treat + (job_seek), 
               n.iter = 10000)
```
