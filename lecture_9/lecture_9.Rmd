...
```{r}
state_names <- readr::read_csv(file = 'https://raw.githubusercontent.com/fans-stats/intro_to_R_stats_2022/main/lecture_9/us_states.csv') %>% 
  dplyr::rename('state' = 'Abbreviation', 
                'region' = 'State name')
```
...
```{r}
dataset %>% 
  dplyr::inner_join(state_names) %>% 
  dplyr::inner_join(states_map) %>% 
